...
from model2mmwave import MMWaveNet
...
def train_wifi(dataset_r, config_t):
    # [17,3] [17, 3]
    ...
    train_dataset, val_dataset = make_dataset(dataset_r, config)
    ...
    if not config_t['weight_path']:
        model_weight_path = config_t['weight_path']
        print("using file {} pre-train.".format(model_weight_path))
        net.load_state_dict(torch.load(model_weight_path, map_location='cpu'))
...
def train_mmwave(dataset_r, config_t):
    log_dir_name = datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S')
    tb_writer = SummaryWriter(log_dir=os.path.join('logsMMWave', log_dir_name))
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            val_bar = tqdm(val_loader, file=sys.stdout, leave=False)
            for batch_idx, batch_data in enumerate(val_bar):
                val_features = batch_data['input_mmwave']
                val_labels = batch_data['output']
                predict_y = net(val_features.to(device))
                cpu_predict_y = predict_y.cpu()
                mpjpe, pampjpe = calulate_error(cpu_predict_y.numpy(), val_labels.numpy())
...
if __name__ == '__main__':
    ...
    if config['modality'] == 'wifi-csi':
        train_wifi(dataset_root, config)
    elif config['modality'] == 'mmwave':
        train_mmwave(dataset_root, config)
    else:
        print('config modality error')
