            ...
            for action in actions:
                if self.data_unit == 'sequence':
                    ...
                    for mod in self.modality:
                        data_dict[mod + '_path'] = os.path.join(self.data_base.data_root, self.get_scene(subject),
                                                                subject,
                                                                action, mod)
                    ...
                    for idx in range(frame_num):
                        ...
                        for mod in self.modality:
                            data_dict[mod + '_path'] = os.path.join(self.data_base.data_root, self.get_scene(subject),
                                                                    subject, action, mod,
                                                                    "frame{:03d}".format(idx + 1) + self.get_data_type(
                                                                        mod))
        ...
        if mod in ['infra1', 'infra2', 'rgb']:
            ...
            for i in range(10):  # 32
                ...
                if nan_num != 0:
                    temp_not_nan_col = temp_col[temp_col == temp_col]
                    temp_col[np.isnan(temp_col)] = temp_not_nan_col.mean()
            data2 = (data - np.min(data)) / (np.max(data) - np.min(data))  # [3, 114, 10]
